...
//   SimpleCocoaServer.m
//   ------------------------------------------------------
//  | Created by David J. Koster, release 21.04.2008.      |
...
{
	//NSLog(@"%d", [connections count]);
	NSString *send = [[NSString alloc] initWithFormat:@"a%@a",string];
	BOOL result = [self sendData:[send dataUsingEncoding:NSWindowsCP1252StringEncoding] toConnection:con];
	[send release];
	return result;
}
...
{
	NSString *send = [[NSString alloc] initWithFormat:@"a%@a",string];
	[self sendDataToAll:[send dataUsingEncoding:NSWindowsCP1252StringEncoding]];
}
...
- (id)initWithFileHandle:(NSFileHandle *)fh delegate:(id)initDelegate
{
    if(self = [super init]) {
		...
		if(addrData) {
			...
			[self setRemoteAddress:[NSString stringWithCString:naddr encoding:NSWindowsCP1252StringEncoding]];
			CFRelease(addrData);
		} else {
...
- (void)dataReceivedNotification:(NSNotification *)notification
{
	...
	if ([data length] == 0) {
		...
		NSString *received = [[NSString alloc] initWithData:data encoding:NSWindowsCP1252StringEncoding];
